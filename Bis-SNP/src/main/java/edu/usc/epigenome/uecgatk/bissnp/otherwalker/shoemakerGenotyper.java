...
public class shoemakerGenotyper extends LocusWalker<Long, Long> implements TreeReducible<Long> {
	...
	public static double[] normalizeToLog10(double[] array, boolean takeLog10OfOutput, boolean keepInLogSpace, boolean ignoreInfiPos) {
		...
		Double maxValue = Collections.max(Arrays.asList(ArrayUtils.toObject(array)));
	...
	public Long map(RefMetaDataTracker tracker, ReferenceContext ref, AlignmentContext context) {
		...
		DiploidGenotype bestGenotypeIndex = DiploidGenotype.createHomGenotype(BaseUtils.Base.A.base);
		;
		DiploidGenotype secondBestGenotypeIndex = DiploidGenotype.createHomGenotype(BaseUtils.Base.A.base);
		;
	...
	private void add(double[] nucleotideProbabilityMatrix, PileupElement p) {
		...
		if (BaseUtils.basesAreEqual(base, BaseUtils.Base.A.base)) {
		...
		} else if (BaseUtils.basesAreEqual(base, BaseUtils.Base.C.base)) {
		...
		} else if (BaseUtils.basesAreEqual(base, BaseUtils.Base.G.base)) {
		...
		} else if (BaseUtils.basesAreEqual(base, BaseUtils.Base.T.base)) {
		...
		for (PileupElement p : pileup) {
			...
			while (i <= baseCountToCheckUpDownStream) {
				...
				if (i == 0) {
					if (p.getRead().getReadNegativeStrandFlag()) {
						baseRev[4]++;
						if (BaseUtils.basesAreEqual(p.getBase(), BaseUtils.Base.A.base)) {
							baseRev[0]++;
						} else if (BaseUtils.basesAreEqual(p.getBase(), BaseUtils.Base.C.base)) {
							baseRev[1]++;
						} else if (BaseUtils.basesAreEqual(p.getBase(), BaseUtils.Base.G.base)) {
							baseRev[2]++;
						} else if (BaseUtils.basesAreEqual(p.getBase(), BaseUtils.Base.T.base)) {
							baseRev[3]++;
						}
					} else {
						baseFwd[4]++;
