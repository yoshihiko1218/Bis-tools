...
public class BisulfiteGenotyperEngine {
	...
	//private static final HashSet<String> filter = new HashSet<String>(20);
	...
	public BisulfiteGenotyperEngine(RefMetaDataTracker tracker, ReferenceContext refContext, AlignmentContext rawContext, BisulfiteArgumentCollection BAC, GenomeAnalysisEngine toolkit) {
		...
		//filter.add(GATKVCFConstants.LOW_QUAL_FILTER_NAME);
	...
	public VariantContext calculateLikelihoods(RefMetaDataTracker tracker, ReferenceContext refContext, HashMap<String, AlignmentContext> stratifiedContexts, AlignmentContextUtils.ReadOrientation type,
	...
	public void calculateLikelihoodsAndGenotypes(RefMetaDataTracker tracker, ReferenceContext refContext, AlignmentContext rawContext) {
		...
		HashMap<String, AlignmentContext> stratifiedContexts = BisSNPUtils.splitContextBySampleName(rawContext);
	...
	private void assignGenotypes(VariantContext vc, HashMap<String, Integer> bestAFs, GenotypesContext calls, HashMap<String, Double> logRatios, HashMap<String, BisulfiteContextsGenotypeLikelihoods> BCGLs,
		...
		for (String sampleName : sampleNames) {
			...
			//calls.add(new Genotype(sampleName, myAlleles, logRatio, null, g.getExtendedAttributes(), false));
			Genotype newGenotype = new GenotypeBuilder(sampleName,myAlleles).log10PError(logRatio)
					.AD(g.getAD()).DP(g.getDP()).attributes(g.getExtendedAttributes()).make();
			calls.add(newGenotype);
		}
	...
	private BisulfiteVariantCallContext calculateGenotypes(RefMetaDataTracker tracker, ReferenceContext refContext, AlignmentContext rawContext, HashMap<String, AlignmentContext> stratifiedContexts,
			HashMap<String, BisulfiteContextsGenotypeLikelihoods> BCGLs, VariantContext vc) {
		// initialize the data for this thread if that hasn't been done yet
		if (bglcms.get() == null) {
			return null;
		}
		...
		String cType = null;
		HashSet<String> cytosineConfirmed = new HashSet<String>(10);
		HashSet<String> cytosinePatternInRef = new HashSet<String>(10);
		HashMap<String, Double> logRatiosMap = new HashMap<String, Double>();
		HashMap<String, Integer> bestAFsMap = new HashMap<String, Integer>();
		...
		if (passesCallThreshold(logRatio) & !cytosineConfirmed.isEmpty()) {
			...
			if(BaseUtils.basesAreEqual(refContext.getBase(), BaseUtils.Base.G.base)){
				attributes.put(BisulfiteVCFConstants.C_STRAND_KEY, '-');
			}
			else if(BaseUtils.basesAreEqual(refContext.getBase(), BaseUtils.Base.C.base)){
				attributes.put(BisulfiteVCFConstants.C_STRAND_KEY, '+');
			}
		}
		...
		if(!passesCallThreshold(logRatio)){
			vcb.filters(GATKVCFConstants.LOW_QUAL_FILTER_NAME);
		}else{
			vcb.passFilters();
		}
		...
		//double log10perror = -logRatio / 10.0;
		//if(log10perror > 99){
		//	log10perror = 99;
		//}
	...
	private VariantContext createVariantContextFromLikelihoods(ReferenceContext refContext, Allele refAllele, HashMap<String, BisulfiteContextsGenotypeLikelihoods> BCGLs) {
		...
		LinkedHashSet<Allele> allelesSet = new LinkedHashSet<Allele>(10);
		...
		for (Entry<String, BisulfiteContextsGenotypeLikelihoods> BCGLEntrySet: BCGLs.entrySet()) {
			...
			*/
			...
			Genotype newGenotype = new GenotypeBuilder(BCGL.getSample(),thisSampleAllele).attributes(attributes).make();
